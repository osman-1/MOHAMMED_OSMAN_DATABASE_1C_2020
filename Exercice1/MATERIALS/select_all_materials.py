...
try:
    ...
    mysql_select_string = "SELECT * FROM t_material ORDER BY id_material ASC"
    ...
    for row in records_select:
        print(row['id_material'],row['material'])

    for row in records_select:
        output = "id: {id_material}  genre: {material}"
